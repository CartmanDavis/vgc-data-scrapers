...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  top_cut_mega  AS (
    ...
    FROM team_mega tm JOIN tc_mp ON tc_mp.team_id = tm.team_id
  ),
  base AS (
  ...
  top_cut AS (
    SELECT tm.item AS pokemon, COUNT(DISTINCT tm.team_id) AS teams,
      ROUND(SUM(tcmp.score) * 100.0 / NULLIF(COUNT(tcmp.score), 0), 2) AS win_rate
    FROM team_mega tm JOIN tc_mp tcmp ON tcmp.team_id = tm.team_id
    GROUP BY tm.item
  )
  SELECT
    b.pokemon, b.teams,
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  top_cut_total AS (
    ...
    FROM team_combos tc JOIN tc_mp ON tc_mp.team_id = tc.team_id
  ),
  base AS (
  ...
  top_cut AS (
    SELECT tc.combo, COUNT(DISTINCT tc.team_id) AS teams,
      ROUND(SUM(tcmp.score) * 100.0 / NULLIF(COUNT(tcmp.score), 0), 2) AS win_rate
    FROM team_combos tc JOIN tc_mp tcmp ON tcmp.team_id = tc.team_id
    GROUP BY tc.combo
  )
  SELECT
