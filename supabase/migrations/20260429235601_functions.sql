...
-- ─── Drop old overloads (no p_since param) ───────────────────────────────────
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  WHERE tour.format = 'M-A'
    ...
    AND (p_since IS NULL OR tour.date >= p_since)
    AND (p_mode = 'all'
      OR (p_mode = '4plus' AND EXISTS (
            SELECT 1 FROM tournament_standings ts
            WHERE ts.tournament_id = t.tournament_id AND ts.player_id = t.player_id AND ts.wins >= 4))
      OR (p_mode = 'topcut' AND EXISTS (
            SELECT 1 FROM top_cut_teams_ma(p_since) tc WHERE tc.team_id = t.id)))
