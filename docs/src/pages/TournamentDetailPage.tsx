  ...
  return (
    <div className="list-page">
      ...
      <div className="list-page__header">
        <div>
          <h1 className="list-page__title">{meta?.name ?? `Tournament ${id}`}</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
            {meta && (
              <span style={{ fontFamily: 'var(--font-data)', fontSize: 12, color: 'var(--text-4)' }}>
          ...
          {loading
            ? SKEL_WIDTHS.map((w, i) => (
              <tr key={i} className="profile-skel-row" aria-hidden>
                <td><div className="skel" style={{ width: 24, height: 20 }} /></td>
                ...
                <td><div className="skel" style={{ width: '85%', height: 13 }} /></td>
              </tr>
            ))
            : rows.map(r => (
              <tr key={r.placing}>
                <td style={{ fontFamily: 'var(--font-data)', fontSize: 18, textAlign: 'center', lineHeight: 1 }}>
                  {r.placing <= 3 ? medal(r.placing) : (
                    <span style={{ fontFamily: 'var(--font-data)', fontSize: 13, color: 'var(--text-3)' }}>{r.placing}</span>
                  )}
                </td>
                <td className="profile-table__name">
                  <Link to={`/players/${r.player_id}`} className="cell-link">{r.player_name}</Link>
                </td>
                <td>
